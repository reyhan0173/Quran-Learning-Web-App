...
const getAssignedText = async (studentId, courseId) => {
  try {
    ...
    console.log(data.approvalStatus, data.approvalStatus, data.approvalStatus, data.approvalStatus);
    return data.approvalStatus ? "Assign" : "Re-Assign";
...
export default function ({ studentId, courseId, isAssigned, setIsApproved, onAssignClick }) {
  ...
  useEffect(() => {
    const fetchAssignedText = async () => {
      setAssignText(await getAssignedText(studentId, courseId));
    };

    fetchAssignedText();
  }, [studentId, courseId, isAssigned]); // Re-run effect if studentId or courseId changes
